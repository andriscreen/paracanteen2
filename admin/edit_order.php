...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($days as $day) {
            $makan_field = "makan_$day";
            $kupon_field = "kupon_$day";
            $libur_field = "libur_$day";
            
            $makan_value = isset($_POST[$makan_field]) ? 1 : 0;
            $kupon_value = isset($_POST[$kupon_field]) ? 1 : 0;
            $libur_value = isset($_POST[$libur_field]) ? 1 : 0;
            
            // Validasi: hanya satu yang bisa dipilih
            if ($makan_value + $kupon_value + $libur_value > 1) {
                throw new Exception("Hari " . ucfirst($day) . " hanya boleh memilih satu opsi!");
            }
            
            $update_fields[] = "$makan_field = ?";
            $update_fields[] = "$kupon_field = ?";
            $update_fields[] = "$libur_field = ?";
        ...
        foreach ($days as $day) {
            ...
            $new_kupon_total += $_POST["kupon_$day"] ?? 0;
        }
        
        // Di bagian update kupon history di edit_order.php
        if ($old_kupon_total != $new_kupon_total) {
            $kupon_diff = $new_kupon_total - $old_kupon_total;
            
            // Update kupon history
            ...
            // Update total kupon user - PASTIKAN menggunakan +/-
            ...
            // Log perubahan
            error_log("Updated order #$order_id: Kupon changed from $old_kupon_total to $new_kupon_total (diff: $kupon_diff) for user #{$order['user_id']}");
        }
                                                        ...
                                                        <div class="col-md-4 mb-3">
                                                            <div class="card">
                                                                ...
                                                                <div class="card-body">
                                                                    ...
                                                                    <input type="hidden" name="makan_<?= $day_en ?>" id="hidden_makan_<?= $day_en ?>" value="<?= $current_makan ?>">
                                                                    <input type="hidden" name="kupon_<?= $day_en ?>" id="hidden_kupon_<?= $day_en ?>" value="<?= $current_kupon ?>">
                                                                    <input type="hidden" name="libur_<?= $day_en ?>" id="hidden_libur_<?= $day_en ?>" value="<?= $current_libur ?>">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <?php endforeach; ?>
    ...
    <script>
        ...
        document.querySelectorAll('.day-radio').forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                // Reset all hidden fields for this day
                document.getElementById(`hidden_makan_${day}`).value = '0';
                document.getElementById(`hidden_kupon_${day}`).value = '0';
                document.getElementById(`hidden_libur_${day}`).value = '0';
                
                // Set the selected value
                document.getElementById(`hidden_${value}_${day}`).value = '1';
